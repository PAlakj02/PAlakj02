
---

<h1 align="center">Hi 👋I'm Palak</h1>
<h3 align="center"> Machine Learning,AL,LLMs,RAgs,Apps </h3>

---

🎓 **Curious Mind @ IIT Roorkee**  
🚀 Building bridges across  🤖 Machine Learning, 🧬 Biotechnology

---

### 🔍 What I'm Working On

-🧠 Trustworthy & Explainable AI :— Designing interpretable ML systems for credit risk assessment and ethical decision-making in LLMs.

-⚛️ Physics-Informed Neural Modeling :— Leveraging PINNs to solve quantum-scale differential equations with enhanced physical fidelity.

-🧬 Computational Biology & Protein Modeling :— Applying AI-driven methods to study and simulate intrinsically disordered proteins.

-🌐 Agentic AI & Automation Systems :— Developing LLM-integrated tools and scalable backend architectures for intelligent digital workflows.


⚡ _Fun Fact:_ I name my bugs before debugging them. They deserve a proper send-off.

---

### 🛠️ My Toolbox

<div align="center">

<!-- Programming -->
<img src="https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-C++-00599C?logo=c%2B%2B&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-SQL-4479A1?logo=postgresql&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-JavaScript-F7DF1E?logo=javascript&logoColor=black&style=for-the-badge" />

<!-- ML / DL -->
<img src="https://img.shields.io/badge/-Scikit%20Learn-F7931E?logo=scikit-learn&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-TensorFlow-FF6F00?logo=tensorflow&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-PyTorch-EE4C2C?logo=pytorch&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-HuggingFace-FFD21F?logo=huggingface&logoColor=black&style=for-the-badge" />

<!-- Web Dev -->
<img src="https://img.shields.io/badge/-HTML5-E34F26?logo=html5&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-CSS3-1572B6?logo=css3&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=black&style=for-the-badge" />

<!-- Tools -->
<img src="https://img.shields.io/badge/-Git-F05032?logo=git&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-VS%20Code-007ACC?logo=visualstudiocode&logoColor=white&style=for-the-badge" />
<img src="https://img.shields.io/badge/-Jupyter-F37626?logo=jupyter&logoColor=white&style=for-the-badge" />

</div>

---

### 🔬 Featured Projects

- [Credit Card Default Predictor](https://github.com/PAlakj02/credit-risk-ml)  
  Detect credit default risks using LightGBM, SMOTE, and advanced feature engineering.

- [PINNs for Schrödinger Equation](https://github.com/PAlakj02/PINNSolver.git)  
  Physics-Informed Neural Network model to solve time-dependent quantum systems with PDEs.

- [readingmyfavbooks](https://github.com/PAlakj02/readingmyfavbooks.git)  
  A full-stack extension + LLM-powered app that tracks, summarizes, and archives online reading.

---


---

### 🤝 Connect with Me

<p align="center">
  <a href="https://www.linkedin.com/in/palakjagtap" target="_blank">
    <img src="https://img.shields.io/badge/-LinkedIn-0A66C2?logo=linkedin&logoColor=white&style=for-the-badge" />
  </a>
  <a href="mailto:palakjagtapp93@email.com">
    <img src="https://img.shields.io/badge/-Email-D14836?logo=gmail&logoColor=white&style=for-the-badge" />
  </a>
</p>

---

> ✨ “Build what you love, share what you know.”  
> _Like what you see? Drop a ⭐ to show some love._

---

